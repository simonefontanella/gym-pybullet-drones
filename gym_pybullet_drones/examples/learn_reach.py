...
from gym_pybullet_drones.envs.single_agent_rl.ReachThePointAviary import ReachThePointAviary
from gym_pybullet_drones.utils.utils import sync, str2bool

DEFAULT_RLLIB = True
DEFAULT_GUI = True
DEFAULT_RECORD_VIDEO = True
DEFAULT_OUTPUT_FOLDER = 'results/singleDrone/'
DEFAULT_COLAB = False
...
def run(rllib=DEFAULT_RLLIB,output_folder=DEFAULT_OUTPUT_FOLDER, gui=DEFAULT_GUI, plot=True, colab=DEFAULT_COLAB, record_video=DEFAULT_RECORD_VIDEO):
    env_name = "reachthepoint-aviary-v0"
    from gym.envs.registration import register

    register(
        id=env_name,
        entry_point='gym_pybullet_drones.envs.single_agent_rl.ReachThePointAviary:ReachThePointAviary',
    )

    register_env(env_name, lambda _: ReachThePointAviary())
    ...
    for i in range(100000): # Typically not enough
        results = agent.train()
    ...
    env = ReachThePointAviary(gui=gui,
                        record=record_video
                        )
